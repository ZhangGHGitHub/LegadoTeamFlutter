...
- 子代理默认路由（任务类型 → 角色子代理）：故障/bug 报告先派 `debug-engineer` 复现与根因定位；代码实现/修改派 `full-stack-engineer`；代码审查派 `code-reviewer`（改动提交前必经审查）；测试与验收派 `qa`；大范围代码调研/依赖梳理派 `researcher`；UI/浏览器端到端验证派 `ui-operator`。常规任务先判断能否派发（可自包含、上下文隔离有价值、可并行），能派则派
