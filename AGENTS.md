...
- **模拟器验证档（2026-09-20 用户指令：改用 MuMu，弃用雷电）**：
  ...
  - **第二车道 = MuMu 实例 index 0「test2」（2026-09-20 用户开通并改名，用于并行设备任务）** → 端点 **`127.0.0.1:16384`**（宿主转发；与 Test 的 guest 直连端点不同）。已由主代理配置为**与 Test 同规格**：`wm size 1080x1920`、`wm density 480`（物理 720x1280，**override 不持久**，实例重启后需重跑这两条），并已装齐三包（我方 / 原版 3.26082823 / 参考版 kazusa 3.26.15）。**用途**：与 Test 并行跑设备任务（参考采集、双/三包对比、独立验证），避免单机串行；**Test 仍是主验证档**（自测与用户验收语义不变）。
  - **MuMu ROM 限制与绕行（2026-09-20 实测 + 用户侧复核，派任务前必读）**：设备伪装三星 SM-G9900/Android 15，模拟器特征 prop 全空。**中文输入三条绕行路径（不要再撞 IME）**：① 批量书源验证走 FFI/example——`cargo run --release --example scan_search_sources -- <db> @关键词文件(UTF-8 无 BOM) <报告.json>`（全程不碰模拟器）；② 走 UI 时把中文注入 `search_keywords`（该机 **adbd 即 root**、自带 sqlite3 3.44.3，`/data/data/<pkg>/app_flutter/legado.db` 直写；写完退出搜索页再进即刷新，不必 force-stop；WAL 下勿高频外部连接）；③ Flutter 测试内 `tester.enterText()` 直进引擎、绕过系统 IME。其余限制：IME enable/set 被安全策略 patch、`input text` 仅 ASCII；`cmd clipboard` 有命令但空实现（`service call clipboard` 可通但构造 ClipData 过繁、不值得走）；`screencap` 不能写 `/sdcard`（用 `exec-out screencap -p`）；`uiautomator` 写转储后自杀（噪声）；`content` 与 toybox `grep` 段错误（改本地解析）；`settings put secure` 静默回滚（用 `global`）；logcat 被 `E MESA: Failed to find VkFence` 刷屏。
