...
- **模拟器验证档（2026-09-20 用户指令：改用 MuMu，弃用雷电）**：
  ...
  - **第二车道（2026-09-20 用户开通，用于并行设备任务）**：MuMu 实例 index 0「看刘大?」→ 端点 **`127.0.0.1:16384`**（宿主转发；与 Test 的 guest 直连端点不同）。已由主代理配置为**与 Test 同规格**：`wm size 1080x1920`、`wm density 480`（物理 720x1280，**override 不持久**，实例重启后需重跑这两条），并已装齐三包（我方 / 原版 3.26082823 / 参考版 kazusa 3.26.15）。**用途**：与 Test 并行跑设备任务（参考采集、双/三包对比、独立验证），避免单机串行；**Test 仍是主验证档**（自测与用户验收语义不变）。
