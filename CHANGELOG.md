...
### Fixed
- [UI] **搜索结果可见性口径修订（彻底修复结果被搜索历史层遮挡一类问题）**：帮助层（历史/联想）改为「编辑新查询时才显示」——输入框文字与已搜关键词不一致、或无结果、或空词时显示；结果一旦存在且对应当前关键词即保持可见（含重新聚焦、横幅展开收起等一切交互）。原版「聚焦即帮助层」在键盘与焦点解耦的模拟器/桌面上会长期遮挡结果，登记为有意分叉（结果优先）。

## [2.0.311] - 2026-09-26

### Fixed
- [UI] **搜索完成后结果必然可见**：输入帮助层（搜索历史）的显隐口径是「搜索框聚焦即显示」，桌面/模拟器上键盘收起而焦点仍在时，历史层会一直盖住结果且无键盘可收；现搜索完成且有结果时自动收起焦点，结果直接呈现（主动点搜索框仍可唤出历史/帮助层）。

## [2.0.310] - 2026-09-26

### Fixed
- [UI] **搜索失败列表可滚动查看全部条目**：失败横幅展开后此前整块平铺在结果列表上方的固定区，条目多（如 99 条）时超出屏幕的部分被裁切且无法滚动；现限高 45% 视口 + 内部滚动，错误文本长按可选中复制。
