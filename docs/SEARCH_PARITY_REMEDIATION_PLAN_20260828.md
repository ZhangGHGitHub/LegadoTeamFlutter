...
> **【状态 2026-09-20】** 本计划 §三 各阶段已基本实施完毕（逐项核实见 docs/REFACTORING_ACTIVE_PLAN.md 的 P3-6 条目与 §七/§八 实施记录）；F1 loginCheckJs、错误八分类、S0-D 计时、三入口一致性测试已落地（提交 deb0e87005/473968bcaf）。仍开放：explore 链同模式、聚合下沉（可选）、重试/cookie/charset 逐项对齐评估。头部「开放，未实施」措辞作废。
...
| | 重构版 (5556) | 原版 (5558) |
|---|---|---|
| 状态 | **完成**（无「停止搜索」）| 搜索中（慢，逐源下载封面）|
| 总数 | **1556 条** | （进行中）|
| #1 结果 | 斗破苍穹 · 天蚕土豆 · 玄幻 · 简介完整 | 斗破苍穹 · 作者：天蚕土豆 · 武侠仙侠 · 完本 |
| 详情字段 | 「最新：新书元尊已在起点上传…」逐字一致 | 同（逐项匹配）|
| 九支书竹版 | 8456K / 第一千六百四十二章 摇曳生姿（大结局）| **完全相同** |

**结论**：两 app 对同一真实搜索返回**相同核心书籍 + 一致详情字段** → S0 通用搜索路径（列表解析 / 结果聚合 / 真实网络）与原版对齐。此为 P0-1.4 双包基线首个真实夹具证据，也是 S0「依赖真实响应」门槛的落地。

**注意项**：#1 书分类标签差异——重构版「玄幻」vs 原版「武侠仙侠」。疑为各自 #1 源不同（不同源给斗破苍穹打了不同分类值）或分类字段归一化差异，待定位确认。

**剩余 S0 子项**（需带特定特性的定向书源隔离验证；四项均为原版 `WebBook.kt` / `BookList.kt` 已实现、重构版已对齐——如 `error.rs` L41「书源需要登录」对齐原版 `LoginSourceException`）：

| S0 子项 | 原版位置 | 语义（是否登录/验证相关）|
|---|---|---|
| loginCheckJs 成功/失败双路径 | `WebBook.kt:74-98` | **是**——「检测书源是否已登录」，成功/错误响应上都跑该 JS 校验/触发登录 |
| HTTP 重定向最终 URL | `WebBook.kt:99` `checkRedirect` + `baseUrl=res.url` | 否——通用：用跳转后最终 URL 做解析/翻页 |
...
> **⛔ 未完成项标记（唯一遗留）**：S0-C 原版端（5558）终态证据采集——环境级阻断（reverse 僵死/防火墙/无 run-as）,三轮攻坚未闭环；详细根因与三条续作路径见 §8.6 与 §8.8。**该遗留不影响 P0-3 关闭,但 P0-2 S0 保持 DEFERRED。** 其余交接文档未完成项（P0-3 e2e 双机、flutter test 定性、S0-B、S0-E、§7.6.1 G4）均已完成并提交。
...
### 8.8 S0-C 原版端续作诊断（2026-08-29 补充）

第二轮针对原版端（com.legado.app.release @ emulator-5558）的自动化攻坚结果与根因：

1. **导入环节已打通（含根因修复）**：ImportBookSourceDialog 底部「全选（x/7）」为**切换式**按钮——历史轮次中选中数被反复切换为 0/7,确认导入的是 0 个源（原版端搜索从未包含夹具源的根因）。已验证可行序列：深链导入 → 全选（核对勾选态 7 true）→ 确认 → 书源管理可检索到 7 源（`S0C-S0 (S0C) 开启` 等,UI dump 证据）。
2. **网络通道**：LDPlayer 的 adb reverse 会**在数分钟内静默僵死**（规则仍在 reverse --list 中,但设备侧监听消失/转发失效）,且多个残留服务器进程以 SO_REUSEADDR 同抢端口导致日志分流。已验证 8091 设备侧监听存活时导入探针可通（多次 sources_json 落日志）;搜索长跑期间 reverse 死亡 → 请求瞬时失败 → 「分组搜索结果为空」。
3. **范围圈定**：范围对话框分组列表为「分组 tab + 滚动列表」,组 'S0C' 位于列表末尾（滚动可达,已勾选成功过）;导入对话框的自定义分组填写并不改变已存在源的原分组,易造成「选了组但组内无源」的错配。**可行组合**：先完成导入（全选 7/7）→ 搜索页溢出菜单「多分组/书源」→ 分组 tab 滚动勾选 S0C → 确认 → 提交查询按钮（content-desc=提交查询）触发。
...
6. **重要更正（同日深查）**：复查发现,中段轮次里命中夹具服务器的逐源请求**全部来自重构端（5556/8090）**,原版端（5558/8091）在所有历史轮次中**从未产生过任何一条到达夹具服务器的请求**（此前因双服务器共用控制台输出而误归因）。即:原版端的搜索请求从未成功抵达 127.0.0.1/192.168.100.52 的夹具服务,与其-import 拉取（可通,依赖 reverse 新鲜度）形成对照——指向原版搜索执行器对环回地址的请求路径存在独立问题（疑应用内代理/DNS 配置,或书源 searchUrl 落库值与预期不符,需在书源编辑页「搜索」tab 人工核对落库的 searchUrl）。**S0-C 原版端闭合前必须先解决该网络路径问题**,UI 自动化序列本身已不是主要障碍。
7. **连通性终测（同日）**：①模拟器浏览器直接访问 `http://192.168.100.52:8091/marker/...` 与网关 `172.16.1.1:8091` —— 夹具服务器均未收到请求（浏览器 intent 未产生可达连接）,设备→主机直达路径在本环境确认不通;②adb reverse 转发:设备侧监听存在（/proc/net/tcp 可见 :1F9B）,但应用连接**从不**转发到主机监听（监听存在而转发不通,LDPlayer adbd 缺陷实锤）;③结论:5558 环境下原版应用无法与主机夹具服务器建立任何可靠连接,S0-C 原版端采集需更换环境（Windows 防火墙放行+管理员、或真机/其他模拟器、或为原版模块补齐 mavenLocal 构件构建 debug 变体）。
