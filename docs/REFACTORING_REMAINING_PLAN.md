...
- ✅ **v1.37（2026-08-13）增强遗留 C 批销记（v2.0.43）**：§5.14 #1 getSameTitleRemoved/`canRemoveSameTitle`；#4 webdav 流式 PUT；#15 customHosts 直建 Client；#18 loginCheckJs server+UI；缓存任务落库；自定义字体族核销（FontScreen）。**schema v102 仍按计划延后——请用户确认是否强制做**。
