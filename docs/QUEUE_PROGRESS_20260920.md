...
## 二·补  复工后第一批结果（2026-09-21）

- **队列① A 组形态对齐：已收口**（`058ccec1a3` 代码 / `3530f2b33e` 报告 / `cbf4f4cbac` 版本 2.0.301+302；Flutter CI `35565564978` 绿）。要点：
  - A1 全屏扫描器 + 相册本地解码（含 **zxing2 0.2.4 有符号 Int8List 导致 CJK 乱码** 的规避 `_recoverUtf8Text`）；4 例新回归；
  - A5 菜单收敛参考版 5 键（−166 行，reader_screen/排版/翻页零改动）；
  - **A6 确有代码改动**（已核 diff）：深色输入盒填充 `surfaceContainerLow` → `isDark ? onSurface : surfaceContainerLow`，提示/图标色改 `isDark ? surface : onSurfaceVariant`（对齐参考"深色下浅色盒"）；**待设备像素复核**（若标准引擎深色盒实为 #1E1B13 则回退 sCL）；
  - A4 取证：参考（Compose）单击恒进阅读器 / 长按进详情，**我方已一致**；唯一差异"未读书单击进详情"为我方自创回退 → 保留 + 登记（**待用户小决**）；
  - A2 事实清单：参考 14 模式 vs 我方 13 套；**名称 9 处不一致**、顺序不一致；**深色色值 11/12 完全一致**；唯 Transparent 深色 sCL 参考 0x8F000000 vs 我方 #00FFFFFF（源码未显式设 → 需重采复核）。
- **队列⑦ 可拆两步并行**：⑦a **A2 改名/排序对齐**（纯代码，依据已从参考源码得到，**不需设备**）＋ ⑦b **设备重采三项**（A3 默认主题外观页干净基线、Transparent 深色 sCL、A6 输入盒像素 —— 放 Test2）。
- 排障经验：推送时若代理通道 SSL 握手失败，用 `git -c http.proxy= -c https.proxy= push` **直连可通**。
...
| 序 | 任务 | 备注 |
|---|---|---|
| ① | **[已完成 2026-09-21]** **A 组续跑**：先验在途改动可编译（analyze/test）→ 补新依赖评估 → A4 先取证后改 → A1/A5/A6 收尾并跑门禁 | 复用 §2.1 的在途文件，勿重写 |
| ② | **[已完成 2026-09-21，全项通过]** **M1 验证收尾**：按 §2.2 待办清单跑完 + 写小结 | 设备：Test `192.168.1.19:5555` |
| ③ | **[已完成 2026-09-21，已过 code-reviewer]** A-4 死默认改必填（含 code-reviewer）/ A-5 删 `app_colors.dart`（先验无动态引用→在用槽位改 MD3→全量测试）/ N6 保留页面+收起入口+登记授权偏离 | 用户已裁决 |
| ③b | **A4 书架单击对齐（用户裁决 B 方案/重构版模式）**：单击 → 详情页+立即自动开读；阅读器零进度兜底；code-reviewer + 设备验证 | 待派（槽位空出即派） |
| ⑦c | **A3 外观页 IA 对齐**（基线已入库，待实施） | 待派 |
| ⑦d | **A6 搜索输入盒回退 sCL** | **已完成（本批提交）** |
...
| ⑦ | **[⑦b 进行中(Test2) / ⑦a 待派]** A2/A3 设备轮：参考 12 主题卡名称/颜色事实提取 + 外观页干净基线重采 → 对齐（视觉照参考版） | 建议放 **Test2** |
...
| ⑫ | **P3 既有行为（审查发现，登记）**：书架从阅读器返回不刷新（内存 Book 进度陈旧；仅下拉刷新）——影响小（详情页取 DB 进度），可低优修 | 待排 |
