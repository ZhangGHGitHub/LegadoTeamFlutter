...
>
> **验收口径修订（2026-08-06，Task #122）**：批次 1-3 以 `flutter analyze` 0 error + 全量回归（1117 用例）为验收口径；新增组件的 widget 测试未随批次同步补齐，**显式列为后续治理批次的补强项**（非默认跳过）；历史批次任务卡中「补 widget 测试」步骤按此口径修订执行。
>
