...
### Fixed
- [Rust] **修复两个热门书源搜索失败**：①「七猫小说·API」的搜索签名需要 `java.security.MessageDigest`（MD5/SHA 摘要）——已按能力清单补上（支持 MD5/SHA-1/SHA-256/SHA-512，未知算法仍有明确提示）；②「77读书」的搜索/正文规则用到 jsoup 的集合写法（`rows.size()` 等）——已补齐，不再报「not a function」。
- 顺带修复：`encodeURIComponent` 对非字符串入参按 JS 规范强转（此前部分书源 URL 模板会因此报类型错误）。

### 登记源侧问题
- 「终极全栖接口」的域名（`cj.rycapi.com`）已在公共 DNS 注销（直连/代理/公共 DNS 三向确认），该源需等书源方更新；
- 「古龙全集」未配置搜索地址，与原版行为一致会提示失败。
