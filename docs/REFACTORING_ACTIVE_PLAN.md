...
- **P2-7 本批实测新发现的三条**（部分关闭，2026-09-18）：**(a) 已关闭**——根因是引擎两处缺陷（裸 `@attr` 取空 + 含 `=` 的括号被当索引区间），已在 `rust/legado-parser/src/html.rs` 修复（提交 `dd842e53d7`）：离线实测网阅小说目录由「1 章垃圾」恢复 **688 条互异 URL**、`meta[property="og:…"]` 由 19 行 junk 恢复 1 行；影响面 21 条规则/15 源；parser 287 测试全绿。**(b) 仍开放**：
  ...
  - **(c) (b) 的根因已定位为 `test_sandbox_memory_limit`（登记未做，2026-09-19，提交 `47552f1064`）**：WSL 复现 3/3 崩溃偏移一致 `ed6b40`（addr2line = `JS_SetPropertyValue` @ quickjs.c:8983，RAX=0 → `mov eax,[rax+0x20]`）——唯一「Windows 跳过、仅 Linux 跑」的 `engine::quickjs_tests::test_sandbox_memory_limit`（rust/legado-js/src/engine.rs:965）在 512KB 堆上限下跑 10 万次大数组写入，QuickJS OOM 路径未校验 NULL 即解引用；崩溃进程唯一未完成用例即它，孤立单跑 300 次 0 崩溃，间歇性来自同进程并行内存压力（非环境 flake、非代码改动引入）。**处置（已做）**：CI 主步骤 `--skip` 该用例（保留其余全部）+ 新增独立步骤 `--exact … --test-threads=1` 串行单跑，止住 CI 间歇红且不丢覆盖。**后续建议（未做）**：① 加 `ulimit -c` + `dmesg` 取证 step（RUST_BACKTRACE 对 C 层 SIGSEGV 无效，(b) 旧建议「RUST_BACKTRACE=1 + 逐用例二分」作废）；② 或 cargo-nextest 每用例一进程；③ 真修复（调低用例严苛度或升级 rquickjs，需 100+ 轮压力验证）；④ cache_store 测试 `set_var`/`remove_var` → 显式注入（并行 UB 卫生项）。
