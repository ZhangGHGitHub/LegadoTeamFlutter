...
- **P2-6 规则引擎 `{{…}}` 模板语义收敛的剩余面**（开放，2026-09-17）：为修「换源到部分书源报『新书源未解析到任何章节』」（`ruleSearch.bookUrl` 多行 JS 链尾段 `{{result}}` 被当选择器解析上一段文本 → 取空 → `web_book.rs` 回退成搜索页 URL → 详情全空 → `tocUrl` 的 `{{$.resourceID}}` 空 → 0 章）已按上游 `AnalyzeRule.SourceRule` 语义（`init`（`:699`）含 `{{}}`/`@get` 置 Mode.Regex、`makeUpRule(result)`（`:773`）用前序结果回填、`getString`（`:355`）`else -> rule` 字面直出、`##` 拆分在回填**之后**）对齐 `rust/legado-parser/src/analyze_rule.rs` + `rust/legado-ffi/src/api/web_book.rs`。收敛过程中经代码审查（两轮复审）确认、**有意留在本批之外或属既存缺口**的八项：
  - **(a) `get_elements` 链内模板段未同步**：`get_elements` / `get_elements_single_step`（`analyze_rule.rs:920` / `:1004`）另有一套内联链实现，Extract 步仍按选择器解析，`{{…}}` 模板段不生效。当前真实库 `toc.chapterList` 含 `{{` 的规则为 0 条（90 条为 JS 链，走 `<js>` 分支不受影响），无实际影响；后续补「getElements 链内模板段」用例 2 条（模板段、模板段后接 JS 段）后再同步。
  - **(b) 非法正则回退口径三处不齐**：parser `apply_hash_replace`（非法正则返回原文）vs 上游 `replaceRegex`（返回 `replacement`）vs FFI `apply_regex_replace`（返回 `replacement`）——旧账；本批只收敛了 replaceFirst **无匹配**（三处统一为 `""`）。
  - **(c) `{{js表达式}}` + 选择器后缀/组合符的语义取舍需单独评审**：上游对「首个 `{{` 位于段首」一律置 Mode.Regex（`{{sel()}}.item` 亦应字面返回），本仓历史上有意保留 G11「展开后按选择器/组合求值」语义（本机库相关规则 16 条）。本批**保持 G11 不动**，是否对齐上游须单独评审并配回归集（先评估那 16 条规则的真实依赖）。
  - **(d) 模板命中时 JS 表达式参数可能执行两次**：模板规则先经 `expand_js_refs`（结果为空则保留原文），模板路径又执行一次 → 空结果场景 `js_calls=2`（复审真 JS 探针 A2）；副作用型参数（`java.put`/`toast`/`ajax`）可能翻倍。当前无 `ajax` 型真实参数命中；建议后续在模板命中时跳过 `expand_js_refs` 的参数展开。
  - **(e) `java.getString` 绑定层不做 JSONPath 分派（用户可见字段值偏差）**：`legado-js/src/host_api/html_parse.rs:296-301` / `quickjs_impl.rs:1815-1827` 的 `java.getString('$.x')` 只做 CSS/HTML 解析，不做 JSONPath；复审真 JS 探针 J1 实证：JSON 内容上 `@js:java.getString('$.a')` → 空。后果（松鹤庭沐等源）：`toc.chapterName` 多出 `🔒`（免费章不应有）、`bookInfo.intro` 标签段为空、kind 行 `…toFixed(1)万字` 得 `0.0`。**规则模板管道本身正确，此项属绑定层缺口，需单独立项**（Kotlin 的 `java.getString` = `AnalyzeRule.getString`，支持全部规则类型）。
  - **(g) 换源后书籍 `bookUrl` 未更新为新源地址（与上游 `toBook()` 的差异，已实测）**：上游 `SearchBook.toBook()`（`data/entities/SearchBook.kt:120-138`）在换源时**用新源候选的 `bookUrl`/`tocUrl` 构造 Book**（即书的键会换成新源地址）；我方 `source_switch.rs:712` 刻意「bookUrl 保持稳定，仅改 origin/originName/tocUrl」（为复用原行主键）。后果实测：换源成功后（`origin/originName` 已更新、712 章已写入、阅读正常），紧接着的详情/目录刷新会拿**旧源地址**按新源规则解析 → `init` 取空 → `tocUrl` 被重推成空参地址（实测落库 `…all-chapter?bookId=`，丢 bookId）→ 后续「刷新目录」会失败。建议：对齐上游在换源事务内同时更新 `bookUrl`（并评估主键变更带来的缓存/书签/阅读进度迁移），或至少在 `tocUrl` 重推路径上加「解析结果为空则保留既有值」的保护。
  - **(f) 琐记（复审记录，0 真实命中，待顺手统一）**：链段模板判定作用于**整段**而非拆分后核心（`analyze_rule.rs:643-645`，与 P0-2 口径不一致；当前 37 条链规则 `T_full==T_core`）；`split_top_level_hash(rule, 0)` 与 `splitn(0)` 行为不一致（当前调用点 4/`usize::MAX` 不可达）；`##regex###` 两段形态 parser 与 FFI 口径分歧（全库 0 命中）；`@js:`/`<js>` 字面出现在 `{{}}` 内部会被误判为 JS 段（无真实命中）。
  - **(h) `elem_analyzer` 缺 `book` JS 绑定（既存，非本批引入）**：analyzer 层有 `with_js_binding("book", …)`，但逐章解析用的 `elem_analyzer` 没有（复审 P3-3 实测：`ruleToc.chapterName` 里 `book.name` 两侧都不生效），上游每章复用同一带 book 绑定的 analyzeRule（`BookChapterList.kt:236-245`）。影响 **5 源**（民间故事/涨姿势/华语中文/月亮小说/可阅文学），与本批 `chapterList` 级依赖书名的 4 源集合不重叠；建议单独立项。
