...
- **P2-7 本批实测新发现的两条**（部分关闭，2026-09-18）：**(a) 已关闭**——根因是引擎两处缺陷（裸 `@attr` 取空 + 含 `=` 的括号被当索引区间），已在 `rust/legado-parser/src/html.rs` 修复（提交 `dd842e53d7`）：离线实测网阅小说目录由「1 章垃圾」恢复 **688 条互异 URL**、`meta[property="og:…"]` 由 19 行 junk 恢复 1 行；影响面 21 条规则/15 源；parser 287 测试全绿。**(b) 仍开放**：
