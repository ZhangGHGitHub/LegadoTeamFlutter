...
| 批 C-II | C1 书源管理复选框批量 / C2 字典规则管理 / C3 字体行内面板 / C4 替换+正则开关（原版对齐） | ✅ C1（2.0.232）/ C3（2.0.233 + 标题字体 2.0.242）/ C4（2.0.229）已完成；**C2 方案已定**（前置取证 2026-09-11：`dict_rule_repository.rs` 已存在且全量 CRUD+seed，缺口=FFI 7 方法 dictRuleList/Add/Update/Delete/SetEnabled/Reorder/Import + 1 页 dict_rule_screen，契约草案已拟，1.5~2 天分三批）待派 |
