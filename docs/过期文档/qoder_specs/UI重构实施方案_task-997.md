...
2. **UI 层混入逻辑**：部分 Provider 内含排序/分组计算逻辑，应下沉 Rust
...
**铁律**：
- Screen/Widget 中**禁止**直接调用 `BookApi`
- Notifier 中**禁止**包含 UI 逻辑（如动画控制、ScrollController）
- 所有业务计算（排序、过滤、规则解析）通过 `BookApi` 委托 Rust
...
| 任务 | 产出 | 验收标准 |
|------|------|----------|
| 3.1 实现 `ExploreNotifier` | providers/explore/ | 分类解析正确 |
| 3.2 发现页：书源列表 + 分类 Tab + 书籍网格 | explore_screen 重构 | 对齐 ExploreFragment.kt |
| 3.3 搜索：多源并行 + 进度指示 + 结果合并 | search_screen 重构 | 搜索结果与原版一致 |
| 3.4 搜索历史 + 联想 | 本地存储 | 交互流畅 |
...
- **UI 轨**负责本方案所有产出（screens/widgets/providers/theme/utils）
- **Rust 轨**负责 `rust_api.dart` 新增方法 + FFI 编译
- 新增 API 需求走 `API_CONTRACT.md` § 3 登记 → Mock 先行 → Rust 交付 → 验证切换
