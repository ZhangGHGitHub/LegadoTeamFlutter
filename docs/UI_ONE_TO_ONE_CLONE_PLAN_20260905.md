...
## 五·十、全屏差异清单（2026-09-07 实测取证）

主框架五页签（首页/书架/发现/订阅/我的）+ 二级页（搜索/设置主页/书籍详情）已完成双机实机配对取证，完整差异与处置建议见 [UI_SCREEN_DIFF_INVENTORY_20260907.md](UI_SCREEN_DIFF_INVENTORY_20260907.md)。要点：①缺「首页」页签（最近阅读+阅读统计+今日目标表盘+首页模块管理）；②我的页缺 Web 服务开关/字典规则/标签规则/缓存管理/AI 对话入口与分组头；③设置主页为参考版集中式 8 组 vs 我方分散式；④搜索历史整行卡 vs 流式 chip；⑤书籍详情宫格构成与封面布局差异。待补采：目录/换源/书源管理/缓存/外观/备份/字体/朗读/批量态/发现二级页。
