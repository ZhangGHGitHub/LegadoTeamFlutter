...
## 五·九、主菜单布局差异登记（2026-09-06，后续批次）

参考版主菜单（用户截图）为单一底部面板：亮度条 + 两侧箭头章节滑条 + 五项行（章节梗概 / AI 改写 / 全文搜索 / 自动翻页 / 目录），无图标行与全文搜索独立 chip；另有仅「朗读/设置」角标的精简形态截图（疑为另一状态，待确认）。我方当前保持原版结构（顶栏 + 图标行 + 全文搜索 chip + 目录/朗读/界面/更多底行）。差异处置：① 五项行中「章节梗概 / AI 改写」属已授权 AI 功能按钮占位（按钮占位先行，服务后端独立立项）；② 其余项迁移需与原版对齐红线权衡（原版菜单结构不同）。整体作为独立批次调研后实施，不并入 C 修复。
