...
- 已完成 **14/15**，全部通过 `cargo test -p legado-ffi --features quickjs`（337/0 基线）逐项回归。
- 分支：`feature/rust-parser-gap-fix`，14 个提交（约定式提交 `fix(rust):`）。
- 剩余 1 项（G8 罕见形态）为**深层机制或受并发文件制约**，非单点替换，见 §三精确诊断。
