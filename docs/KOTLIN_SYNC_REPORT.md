...
## 同步窗口 2：2026-07-25 → 2026-08-04（141 个提交）

### 同步执行记录

- 执行日期：2026-08-04
- 同步范围：e1c102803（#396）→ 308ac7b1e（#543），141 个提交
- 提交记录：b10285b8c
- 变更规模：424 文件（新增 138 / 修改 276 / 删除 10），+31,710/-2,235 行
- 执行依据：docs/REFACTORING_REMAINING_PLAN.md §3.5 决策项——选项 A（从 LegadoTeam/legado 合并，先合并再修）

### 功能性变更摘要（约 120 个功能性提交，剔除依赖 bump）

| 功能簇 | 关键提交 | 影响 |
|--------|----------|------|
| 正文高亮体系 | 11+ 提交（圆角/马克笔/半高/底衬/胶囊 + 阴影发光等样式） | 高亮数据模型与样式渲染扩展，涉及 DB 结构变更 |
| Web 段评 | #529/#531 | 网页端段评能力扩展 |
| 应用日志 | #512/#524/#543 | AppLog 日志体系增强 |
| SOCKS5 认证 | #469 | 代理支持用户名/密码认证 |
| 登录 V2 | #402/#488 | 书源登录体系 V2 改造 |
| PDF 导出 | #472/#483 | 文本书/图片书导出 PDF |
| TTS 队列修复 | #461 | HTTP TTS 竞态/队列修复 |
| 音频跳过策略 | #462/#492 | 听书章节跳过策略 |
| MCP 新工具 | #452-#456（5 类新工具） | MCP 套件能力扩展 |
| cURL 转换 | #434 | cURL 命令与请求参数互转 |
