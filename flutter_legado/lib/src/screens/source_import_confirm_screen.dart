...
/// 自定义源分组对话框：controller 生命周期绑定对话框子树，
/// 随子树卸载统一释放（避免退场动画期间 dispose 引发框架断言）；
/// 确定返回（分组名, 是否添加分组），取消返回 null
class _CustomGroupDialog extends StatefulWidget {
  final String initialName;
  final bool initialAddGroup;

  const _CustomGroupDialog({
    required this.initialName,
    required this.initialAddGroup,
  });

  @override
  State<_CustomGroupDialog> createState() => _CustomGroupDialogState();
}

class _CustomGroupDialogState extends State<_CustomGroupDialog> {
