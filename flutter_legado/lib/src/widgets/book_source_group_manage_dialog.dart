...
///
/// 实现策略对齐 [RssGroupManageDialog]：
/// - Flutter FFI 无独立书源分组表，分组从各源 `bookSourceGroup` 推导；
/// - 重命名/删除通过批量 `updateBookSource` 改写各源分组字段；
/// - 「添加分组」收拢未分组源（对标原版 `viewModel.addGroup` / noGroup）；
...
class _BookSourceGroupManageDialogState
  ...
  List<String> get _groups {
    ...
    for (final s in _sources) {
      set.addAll(_splitGroups(s.bookSourceGroup));
    }
    return set.toList();
  }
  ...
  Future<void> _upGroup(String oldGroup, String? newGroup) async {
    ...
    try {
      ...
      for (final s in _sources) {
        final groups = _splitGroups(s.bookSourceGroup);
        ...
        final next = s.copyWith(
          bookSourceGroup: updated.isEmpty ? null : updated.join(','),
        );
        await api.updateBookSource(next);
  ...
  Future<void> _addGroup() async {
    ...
    try {
      ...
      var moved = 0;
      for (final s in _sources) {
        if (_splitGroups(s.bookSourceGroup).isNotEmpty) continue;
        final next = s.copyWith(bookSourceGroup: name);
        await api.updateBookSource(next);
        _sources[_sources.indexOf(s)] = next;
        moved++;
      }
      ...
      if (mounted) {
        ...
        if (moved == 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('没有未分组的书源可归入该分组')),
          );
        }
      }
    } catch (e) {
  ...
  Future<void> _delete(String group) async {
    final confirmed = await showConfirmDialog(
      context,
      title: '删除分组',
      content: '确定要删除分组「$group」吗？分组内书源将移出该分组。',
      confirmText: '删除',
      isDestructive: true,
    );
