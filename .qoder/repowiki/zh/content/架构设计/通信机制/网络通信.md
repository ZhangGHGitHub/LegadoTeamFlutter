...
## 更新摘要
**所做更改**   
- 新增QUIC协议支持章节，详细说明高性能低延迟的网络通信能力
- 更新架构总览图，包含QUIC协议路径
- 增强性能考量部分，添加QUIC优化建议
- 更新依赖关系分析，包含QUIC模块依赖
- 扩展故障排查指南，包含QUIC相关问题的诊断方法
...
## 简介
本文件面向Legado项目的网络通信子系统，系统性阐述HTTP客户端实现与HTTPS安全通信。重点覆盖连接池管理、请求拦截器、重试机制、超时控制、缓存策略、并发控制、代理配置、DNS解析、网络状态监听、性能优化、流量监控与故障诊断，以及错误分类与用户友好提示。**最新更新**：新增QUIC协议支持，提供高性能低延迟的网络通信能力，增强了网络层的协议支持。文档以代码级分析为基础，辅以架构图与时序图，帮助读者快速理解并高效使用或扩展该网络层。
...
## 架构总览
下图展示从Android调用到Rust网络库执行的端到端流程，包括拦截器链、重试、代理、TLS、限速、Cookie等关键路径，**新增QUIC协议支持**。
...
- 常见问题定位
  ...
  - 5xx错误：查看服务端日志与重试策略
  - **QUIC连接失败：检查网络环境是否支持QUIC，确认防火墙规则**
- 诊断手段
  - 启用详细日志（请求/响应摘要、耗时、状态码）
  - 抓取网络包（谨慎使用自签CA）
  - 源站健康检查与延迟统计
  - **QUIC协议诊断：检查QUIC握手过程与连接状态**
- 用户友好提示
  - 将技术错误映射为用户可读消息（如“网络不可用”“请检查代理设置”）
  - 提供重试按钮与离线缓存提示
  - **QUIC降级提示：当QUIC不可用时自动降级到TCP/TLS**
...
## 结论
Legado的网络通信子系统以Rust为核心，提供高性能、高可靠的HTTP/HTTPS能力。**最新更新**：新增QUIC协议支持，进一步提升了网络通信的性能与可靠性。通过拦截器链、重试、代理、TLS、限速、Cookie、UA与URL模板等模块化设计，实现了灵活可扩展的请求生命周期管理。**新增的QUIC协议**为移动网络和弱网环境提供了更好的用户体验。结合Android端的集成与工具类，能够覆盖复杂业务场景下的网络需求。建议在生产环境启用严格TLS、合理配置连接池与限流，并通过日志与健康检查持续优化性能与稳定性。**对于需要极致性能的场景，建议启用QUIC协议以获得最佳的网络体验**。
...
- 最佳实践清单
  ...
  - 按域隔离并发与限流，保护自身与服务端
  - 完善错误分类与用户提示，提升体验
  - **优先使用QUIC协议，自动降级到TCP/TLS保证兼容性**
- 参考测试
  - 网络相关用例位于HttpTest.kt，可用于验证基本连通性与错误分支
  - **QUIC协议测试：验证QUIC连接建立、数据传输与降级逻辑**
