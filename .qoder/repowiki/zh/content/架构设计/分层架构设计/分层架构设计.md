# 分层架构设计

<cite>
**本文引用的文件**   
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/lib/cronet/CronetDownloadTask.kt](file://app/src/main/java/io/legado/app/lib/cronet/CronetDownloadTask.kt)
- [flutter_legado/lib/main.dart](file://flutter_legado/lib/main.dart)
- [flutter_legado/flutter_rust_bridge.yaml](file://flutter_legado/flutter_rust_bridge.yaml)
- [rust/legado-ffi/src/lib.rs](file://rust/legado-ffi/src/lib.rs)
- [rust/legado-ffi/src/bridge.rs](file://rust/legado-ffi/src/bridge.rs)
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [modules/web/src/api/index.ts](file://modules/web/src/api/index.ts)
- [modules/web/src/api/axios.ts](file://modules/web/src/api/axios.ts)
- [flutter_legado/lib/src/models/rss_article.dart](file://flutter_legado/lib/src/models/rss_article.dart)
- [flutter_legado/lib/src/services/rust_api.dart](file://flutter_legado/lib/src/services/rust_api.dart)
- [flutter_legado/lib/src/screens/rss_article_detail_screen.dart](file://flutter_legado/lib/src/screens/rss_article_detail_screen.dart)
- [flutter_legado/lib/src/models/reading_stats_today.dart](file://flutter_legado/lib/src/models/reading_stats_today.dart)
- [flutter_legado/lib/src/models/search_result.dart](file://flutter_legado/lib/src/models/search_result.dart)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)
- [flutter_legado/lib/src/providers/reading_stats/reading_stats_state.dart](file://flutter_legado/lib/src/providers/reading_stats/reading_stats_state.dart)
- [flutter_legado/test/unit/reading_stats_provider_test.dart](file://flutter_legado/test/unit/reading_stats_provider_test.dart)
</cite>

## 更新摘要
**变更内容**   
- 增强了模型层架构，新增ReadingStatsToday（今日阅读统计）和SearchResult（搜索结果包装）两个核心数据模型
- 完善了单向依赖结构：models ← services ← providers ← screens
- 更新了Flutter应用的分层架构说明，重点强调了模型层的边界建立和数据契约统一
- 强化了UI组件与服务层解耦的架构原则说明
- 更新了依赖关系分析以反映新的模型层结构

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向Legado应用的分层架构设计与实现，围绕四层职责边界与接口契约展开：表现层（Android Activities/Flutter Widgets/Web Components）、业务逻辑层（Rust核心库）、数据访问层（SQLite/文件系统/网络请求）和外部服务集成层。文档重点说明：
- 各层职责与边界、接口定义
- FFI接口规范与调用约定（Kotlin-Java互操作、Dart-Rust FFI、Web API调用）
- 层间通信机制与错误处理策略（异常传播、错误码映射）
- 分层架构图与数据流图，展示请求处理的完整路径
- 分层设计的优势（解耦、可测试性、可维护性）

**最新更新**：模型层架构得到重要增强，新增ReadingStatsToday（今日阅读统计）和SearchResult（搜索结果包装）两个核心数据模型，完善了单向依赖结构：models ← services ← providers ← screens。通过统一的模型层定义，有效解决了UI组件直接依赖服务层的架构违规问题。

## 项目结构
Legado采用多模块、跨语言的多端统一架构：
- Android应用模块（Kotlin/Java）负责UI与系统能力封装
- Flutter工程（Dart）提供跨平台UI与桥接
- Rust核心库（多个crate）承载业务逻辑、解析、网络、数据库与FFI暴露
- Web管理端（Vue+TS）通过HTTP API与后端交互

```mermaid
graph TB
subgraph "表现层"
A["Android UI<br/>Activities/Services"]
B["Flutter Widgets<br/>Dart"]
C["Web Components<br/>Vue/TS"]
end
subgraph "模型层"
M["数据模型<br/>Book, Chapter, ReadingStatsToday, SearchResult等"]
end
subgraph "业务逻辑层"
D["Rust Core<br/>legado-core"]
E["Rust Book Utils<br/>legado-book"]
F["Rust Parser<br/>legado-parser"]
G["Rust JS Engine<br/>legado-js"]
end
subgraph "数据访问层"
H["SQLite<br/>legado-db"]
I["文件系统"]
J["网络客户端<br/>legado-net"]
end
subgraph "外部服务集成层"
K["Cronet(HTTP加速)"]
L["本地服务器<br/>legado-server"]
M["第三方API/源站"]
end
A --> |Kotlin/Java| D
B --> |Dart FFI| D
C --> |HTTP API| L
D --> H
D --> I
D --> J
J --> K
J --> M
L --> D
B --> M
C --> M
```

图表来源 
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [flutter_legado/lib/main.dart](file://flutter_legado/lib/main.dart)
- [modules/web/src/api/index.ts](file://modules/web/src/api/index.ts)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)

章节来源
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [flutter_legado/lib/main.dart](file://flutter_legado/lib/main.dart)
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [modules/web/src/api/index.ts](file://modules/web/src/api/index.ts)

## 核心组件
- 表现层
  - Android：Activity/Service/ViewModel等UI与系统能力封装，调用Rust FFI或本地服务
  - Flutter：Dart侧Widgets与Provider/State管理，通过Dart-Rust FFI调用核心能力
  - Web：Vue组件与状态管理，通过HTTP API与本地服务器交互
- **模型层（重要增强）**
  - 统一的數據模型定义，包括Book、Chapter、ReadingStatsToday、SearchResult、RssFeedArticle等
  - 通过models.dart统一导出，供UI层和服务层共同引用
  - 解决UI组件直接依赖服务层的架构违规问题
  - 提供稳定的数据契约，支持JSON序列化和反序列化
- 业务逻辑层（Rust核心库）
  - legado-core：领域模型、规则匹配、内容处理、阅读状态、统计等
  - legado-book：电子书格式处理（epub/mobi/pdf/txt/umd）
  - legado-parser：HTML/JSONPath/XPath/正则规则解析
  - legado-js：Rhino脚本引擎与宿主API沙箱
- 数据访问层
  - legado-db：SQLite ORM、迁移、默认数据、仓库模式
  - legado-net：HTTP客户端、Cookie、代理、重试、限速、SSL配置
  - 文件系统：本地缓存、下载、导入导出
- 外部服务集成层
  - Cronet：Android高性能HTTP引擎
  - legado-server：内置HTTP/WS服务，供Web端与管理工具使用
  - 第三方源站/服务：RSS、搜索、TTS等

章节来源
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)

## 架构总览
整体遵循"表现层 → 模型层 → FFI/HTTP → Rust核心 → 数据访问/网络 → 外部服务"的单向依赖与清晰边界。关键要点：
- 表现层仅依赖稳定的接口（FFI函数签名、HTTP API）和模型层，不感知底层实现细节
- 模型层作为统一的数据契约，避免UI层直接依赖服务层
- Rust核心库作为唯一业务中心，保证跨端一致性
- 数据访问层抽象化存储与IO，便于替换与测试
- 外部服务通过适配器隔离，避免污染核心逻辑

```mermaid
sequenceDiagram
participant UI as "表现层(Android/Flutter/Web)"
participant Model as "模型层"
participant FFI as "FFI/HTTP网关"
participant Core as "Rust核心库"
participant DA as "数据访问层"
participant Net as "网络层"
participant Ext as "外部服务"
UI->>Model : 使用数据模型
UI->>FFI : 发起调用(方法/参数)
FFI->>Core : 解码参数并调用业务方法
Core->>DA : 读取/写入持久化数据
Core->>Net : 发起网络请求(必要时)
Net->>Ext : 访问第三方服务
Ext-->>Net : 返回响应
Net-->>Core : 解析响应/错误
Core-->>FFI : 返回结果/错误码
FFI-->>Model : 转换为模型对象
Model-->>UI : 回调/事件更新UI
```

图表来源 
- [rust/legado-ffi/src/lib.rs](file://rust/legado-ffi/src/lib.rs)
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)

## 详细组件分析

### 表现层（Android/Flutter/Web）
- Android
  - 入口与应用初始化：App.kt中完成全局配置、日志、线程池、FFI初始化等
  - 网络加速：CronetDownloadTask封装下载任务，提升大文件/并发下载性能
- Flutter
  - 入口main.dart启动Flutter引擎，加载主题、路由、状态管理
  - 通过flutter_rust_bridge.yaml生成Dart侧FFI绑定，调用Rust核心能力
  - **重要改进**：UI组件现在通过模型层引用数据模型，不再直接依赖服务层
- Web
  - Vue组件通过api/index.ts与axios.ts封装HTTP请求，统一鉴权、错误处理与重试

```mermaid
classDiagram
class AndroidApp {
+初始化()
+配置Cronet()
+注册FFI()
}
class FlutterMain {
+启动引擎()
+加载主题()
+初始化FFI()
+使用模型层()
}
class WebApi {
+请求拦截()
+错误处理()
+重试策略()
}
class ModelLayer {
+统一数据模型()
+ReadingStatsToday()
+SearchResult()
+RssFeedArticle()
+Book()
+Chapter()
}
AndroidApp --> AndroidApp : "依赖"
FlutterMain --> FlutterMain : "依赖"
FlutterMain --> ModelLayer : "使用"
WebApi --> WebApi : "依赖"
```

图表来源 
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/lib/cronet/CronetDownloadTask.kt](file://app/src/main/java/io/legado/app/lib/cronet/CronetDownloadTask.kt)
- [flutter_legado/lib/main.dart](file://flutter_legado/lib/main.dart)
- [flutter_legado/flutter_rust_bridge.yaml](file://flutter_legado/flutter_rust_bridge.yaml)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)
- [modules/web/src/api/index.ts](file://modules/web/src/api/index.ts)
- [modules/web/src/api/axios.ts](file://modules/web/src/api/axios.ts)

章节来源
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/lib/cronet/CronetDownloadTask.kt](file://app/src/main/java/io/legado/app/lib/cronet/CronetDownloadTask.kt)
- [flutter_legado/lib/main.dart](file://flutter_legado/lib/main.dart)
- [flutter_legado/flutter_rust_bridge.yaml](file://flutter_legado/flutter_rust_bridge.yaml)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)
- [modules/web/src/api/index.ts](file://modules/web/src/api/index.ts)
- [modules/web/src/api/axios.ts](file://modules/web/src/api/axios.ts)

### 模型层（重要增强）
**重要架构改进**：为了解决UI组件直接依赖服务层的架构违规问题，建立了独立的模型层，并新增了ReadingStatsToday和SearchResult两个核心数据模型。

- ReadingStatsToday模型
  - 原位置：services/rust_api.dart
  - 新位置：models/reading_stats_today.dart
  - 包含字段：totalSeconds、bookCount、durationSeconds、wordCount、readingSpeed
  - 用于今日阅读统计数据的统一表示

- SearchResult模型
  - 原位置：services/rust_api.dart  
  - 新位置：models/search_result.dart
  - 包含字段：book、sourceName
  - 提供fromSearchBook工厂方法，支持从SearchBook转换

- 模型层职责
  - 定义所有跨层共享的数据结构
  - 提供统一的数据契约
  - 避免循环依赖和架构违规
  - 支持JSON序列化和反序列化
  - 通过models.dart统一导出

```mermaid
classDiagram
class ReadingStatsToday {
+int totalSeconds
+int bookCount
+int durationSeconds
+int wordCount
+double readingSpeed
+构造函数()
}
class SearchResult {
+Book book
+String sourceName
+构造函数()
+fromSearchBook(SearchBook)
}
class ModelsExport {
+export book.dart
+export reading_stats_today.dart
+export search_result.dart
+export rss_article.dart
+export misc.dart
}
ReadingStatsToday --> ModelsExport : "通过models.dart导出"
SearchResult --> ModelsExport : "通过models.dart导出"
```

图表来源 
- [flutter_legado/lib/src/models/reading_stats_today.dart](file://flutter_legado/lib/src/models/reading_stats_today.dart)
- [flutter_legado/lib/src/models/search_result.dart](file://flutter_legado/lib/src/models/search_result.dart)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)

章节来源
- [flutter_legado/lib/src/models/reading_stats_today.dart](file://flutter_legado/lib/src/models/reading_stats_today.dart)
- [flutter_legado/lib/src/models/search_result.dart](file://flutter_legado/lib/src/models/search_result.dart)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)

### 业务逻辑层（Rust核心库）
- legado-core
  - 领域模型：Book、Chapter、Source、Rule等
  - 规则引擎：匹配、转换、渲染
  - 阅读状态与统计：进度、书签、阅读时长
- legado-book
  - 多格式解析与导出：EPUB/MOBI/PDF/TXT/UMD
- legado-parser
  - HTML/JSONPath/XPath/正则规则分析与执行
- legado-js
  - Rhino脚本引擎与宿主API沙箱，支持动态扩展源逻辑

```mermaid
classDiagram
class CoreLib {
+规则匹配()
+内容处理()
+阅读状态()
+统计()
}
class BookUtils {
+解析EPUB()
+解析MOBI()
+解析PDF()
+解析TXT()
+解析UMD()
}
class ParserLib {
+解析HTML()
+解析JSONPath()
+解析XPath()
+正则引擎()
}
class JsEngine {
+初始化引擎()
+执行脚本()
+沙箱隔离()
}
CoreLib --> ParserLib : "使用"
CoreLib --> BookUtils : "使用"
CoreLib --> JsEngine : "使用"
```

图表来源 
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-book/src/lib.rs](file://rust/legado-book/src/lib.rs)
- [rust/legado-parser/src/lib.rs](file://rust/legado-parser/src/lib.rs)
- [rust/legado-js/src/lib.rs](file://rust/legado-js/src/lib.rs)

章节来源
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-book/src/lib.rs](file://rust/legado-book/src/lib.rs)
- [rust/legado-parser/src/lib.rs](file://rust/legado-parser/src/lib.rs)
- [rust/legado-js/src/lib.rs](file://rust/legado-js/src/lib.rs)

### 数据访问层（SQLite/文件系统/网络）
- legado-db
  - SQLite连接、迁移、默认数据注入
  - 仓库模式：BookRepository、ChapterRepository、BookmarkRepository等
- legado-net
  - HTTP客户端封装：请求构建、响应解析、Cookie管理、代理、重试、限速、SSL配置
- 文件系统
  - 缓存、下载、导入导出、临时文件清理

```mermaid
flowchart TD
Start(["进入数据访问"]) --> CheckCache{"是否命中缓存?"}
CheckCache --> |是| ReturnCache["返回缓存数据"]
CheckCache --> |否| ReadDB["查询SQLite"]
ReadDB --> DBOk{"查询成功?"}
DBOk --> |否| FetchNet["发起网络请求"]
DBOk --> |是| UpdateCache["更新缓存"]
FetchNet --> NetOk{"网络成功?"}
NetOk --> |否| HandleError["错误处理"]
NetOk --> |是| ParseData["解析数据"]
ParseData --> SaveDB["写入SQLite"]
SaveDB --> UpdateCache
UpdateCache --> ReturnResult["返回结果"]
HandleError --> ReturnError["返回错误"]
ReturnCache --> End(["结束"])
ReturnResult --> End
ReturnError --> End
```

图表来源 
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)

章节来源
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)

### 外部服务集成层（Cronet/本地服务器/第三方API）
- Cronet：Android高性能HTTP引擎，用于下载与网络加速
- legado-server：内置HTTP/WS服务，为Web端提供统一API
- 第三方API：源站、RSS、TTS、搜索等，通过net层适配

```mermaid
sequenceDiagram
participant UI as "表现层"
participant Server as "本地服务器"
participant Core as "Rust核心"
participant Net as "网络层"
participant Third as "第三方API"
UI->>Server : HTTP请求
Server->>Core : 调用业务方法
Core->>Net : 访问第三方API
Net->>Third : 发送请求
Third-->>Net : 返回数据
Net-->>Core : 解析并返回
Core-->>Server : 业务结果
Server-->>UI : JSON响应
```

图表来源 
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)

章节来源
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)

### FFI接口规范与调用约定
- Dart-Rust FFI
  - 通过flutter_rust_bridge.yaml声明接口，自动生成Dart侧绑定
  - 数据类型映射：基础类型、字符串、字节数组、枚举、结构体
  - 异步调用：Future/Stream，错误以Result或自定义错误码返回
- Kotlin-Java互操作
  - Android侧通过JNI/FFI调用Rust，或使用本地服务HTTP接口
  - 参数序列化与反序列化需严格一致
- Web API
  - 统一RESTful风格，错误码与消息体标准化
  - 鉴权、限流、重试在axios拦截器中实现

```mermaid
sequenceDiagram
participant Dart as "Dart代码"
participant Bridge as "FFI桥接"
participant Rust as "Rust核心"
participant Result as "结果/错误"
Dart->>Bridge : 调用FFI方法(参数)
Bridge->>Rust : 解码并调用
Rust->>Rust : 执行业务逻辑
Rust-->>Bridge : 返回Result(成功/错误)
Bridge-->>Dart : Future/Stream回调
Dart->>Dart : 处理结果/错误
```

图表来源 
- [flutter_legado/flutter_rust_bridge.yaml](file://flutter_legado/flutter_rust_bridge.yaml)
- [rust/legado-ffi/src/lib.rs](file://rust/legado-ffi/src/lib.rs)
- [rust/legado-ffi/src/bridge.rs](file://rust/legado-ffi/src/bridge.rs)

章节来源
- [flutter_legado/flutter_rust_bridge.yaml](file://flutter_legado/flutter_rust_bridge.yaml)
- [rust/legado-ffi/src/lib.rs](file://rust/legado-ffi/src/lib.rs)
- [rust/legado-ffi/src/bridge.rs](file://rust/legado-ffi/src/bridge.rs)

### 错误处理策略
- 表现层
  - Android：统一异常捕获、用户提示、崩溃上报
  - Flutter：错误边界、Toast/Snackbar提示、重试按钮
  - Web：axios拦截器统一错误处理，友好提示与重试
- Rust核心
  - 使用Result类型传递错误，区分业务错误与系统错误
  - 错误码映射到上层（Dart/Kotlin/Web）统一处理
- 数据访问层
  - 网络超时、重试、降级策略
  - 数据库事务回滚、迁移失败处理

```mermaid
flowchart TD
Entry(["错误发生"]) --> Classify{"错误分类"}
Classify --> |网络错误| Retry["重试/降级"]
Classify --> |数据错误| Rollback["回滚/修复"]
Classify --> |业务错误| MapCode["映射错误码"]
Classify --> |系统错误| Report["上报/日志"]
Retry --> Notify["通知用户"]
Rollback --> Notify
MapCode --> Notify
Report --> Notify
Notify --> Exit(["结束"])
```

[此图为概念流程，无需源码映射]

## 依赖关系分析
- 表现层依赖FFI/HTTP接口和模型层，不直接依赖Rust实现
- 模型层作为统一的数据契约，被UI层和服务层共同引用
- Rust核心依赖数据访问与网络层，保持业务纯净
- 数据访问层与外部服务通过适配器隔离，便于替换与测试
- 本地服务器作为Web端的唯一入口，屏蔽底层复杂性

```mermaid
graph LR
UI_A["Android UI"] --> FFI_A["Android FFI"]
UI_F["Flutter UI"] --> FFI_F["Dart FFI"]
UI_W["Web UI"] --> HTTP["HTTP API"]
UI_F --> Model["模型层"]
FFI_A --> Core["Rust核心"]
FFI_F --> Core
HTTP --> Server["本地服务器"]
Server --> Core
Core --> DB["SQLite"]
Core --> Net["网络层"]
Net --> Cronet["Cronet"]
Net --> Third["第三方API"]
```

图表来源 
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
- [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)

章节来源
- [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)

## 性能考量
- 网络优化
  - Cronet加速下载与并发请求
  - 连接池、Keep-Alive、压缩传输
- 缓存策略
  - 多级缓存（内存/磁盘/SQLite）
  - TTL与失效策略
- 解析性能
  - 规则预编译与缓存
  - 流式解析大文件
- 异步与并发
  - Rust侧Tokio协程，Android侧协程/线程池
  - Flutter侧Isolate与后台任务
- **模型层优化**
  - 统一的模型定义减少重复解析开销
  - 通过模型层避免不必要的对象创建
  - ReadingStatsToday和SearchResult的引入减少了跨层数据传输的复杂度

[本节为通用指导，无需源码映射]

## 故障排查指南
- 常见问题
  - FFI调用失败：检查参数序列化、类型映射、版本一致性
  - 网络请求失败：检查代理、SSL配置、重试策略
  - 数据库迁移失败：检查schema版本、默认数据注入
  - **模型层问题**：检查模型定义是否与Rust侧保持一致，确保ReadingStatsToday和SearchResult的正确使用
- 调试手段
  - 启用详细日志，定位错误堆栈
  - 使用本地服务器WebSocket进行实时调试
  - 单元测试与集成测试覆盖关键路径

章节来源
- [rust/legado-ffi/src/lib.rs](file://rust/legado-ffi/src/lib.rs)
- [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
- [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)

## 结论
Legado的分层架构通过清晰的职责边界与稳定的接口契约，实现了跨端一致性与高可维护性。**最新的模型层改进**进一步增强了架构的健壮性，通过ReadingStatsToday和SearchResult等新模型的独立定义，有效解决了UI组件直接依赖服务层的架构违规问题。完善的单向依赖结构：models ← services ← providers ← screens确保了代码的可测试性和可维护性。Rust核心库作为业务中枢，结合SQLite与网络层的抽象，使系统具备良好的扩展性与可测试性。FFI与HTTP API的统一设计，确保了Android、Flutter与Web端的无缝协作。未来可进一步优化缓存策略、解析性能与错误处理体验。

[本节为总结，无需源码映射]

## 附录
- 术语表
  - FFI：外部函数接口
  - Cronet：Android高性能HTTP引擎
  - Repo：仓库模式，数据访问抽象
  - **模型层**：定义跨层共享数据结构的独立层次
  - **ReadingStatsToday**：今日阅读统计数据模型
  - **SearchResult**：搜索结果包装数据模型
- 参考文件
  - [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
  - [flutter_legado/lib/main.dart](file://flutter_legado/lib/main.dart)
  - [flutter_legado/flutter_rust_bridge.yaml](file://flutter_legado/flutter_rust_bridge.yaml)
  - [rust/legado-ffi/src/lib.rs](file://rust/legado-ffi/src/lib.rs)
  - [rust/legado-core/src/lib.rs](file://rust/legado-core/src/lib.rs)
  - [rust/legado-db/src/lib.rs](file://rust/legado-db/src/lib.rs)
  - [rust/legado-net/src/lib.rs](file://rust/legado-net/src/lib.rs)
  - [rust/legado-server/src/lib.rs](file://rust/legado-server/src/lib.rs)
  - [modules/web/src/api/index.ts](file://modules/web/src/api/index.ts)
  - [modules/web/src/api/axios.ts](file://modules/web/src/api/axios.ts)
  - [flutter_legado/lib/src/models/rss_article.dart](file://flutter_legado/lib/src/models/rss_article.dart)
  - [flutter_legado/lib/src/services/rust_api.dart](file://flutter_legado/lib/src/services/rust_api.dart)
  - [flutter_legado/lib/src/screens/rss_article_detail_screen.dart](file://flutter_legado/lib/src/screens/rss_article_detail_screen.dart)
  - [flutter_legado/lib/src/models/reading_stats_today.dart](file://flutter_legado/lib/src/models/reading_stats_today.dart)
  - [flutter_legado/lib/src/models/search_result.dart](file://flutter_legado/lib/src/models/search_result.dart)
  - [flutter_legado/lib/src/models/models.dart](file://flutter_legado/lib/src/models/models.dart)
  - [flutter_legado/lib/src/providers/reading_stats/reading_stats_state.dart](file://flutter_legado/lib/src/providers/reading_stats/reading_stats_state.dart)
  - [flutter_legado/test/unit/reading_stats_provider_test.dart](file://flutter_legado/test/unit/reading_stats_provider_test.dart)