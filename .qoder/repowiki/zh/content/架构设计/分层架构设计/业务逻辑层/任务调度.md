# 任务调度

<cite>
**本文引用的文件**   
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/service/AutoTaskService.kt](file://app/src/main/java/io/legado/app/service/AutoTaskService.kt)
- [app/src/main/java/io/legado/app/model/AutoTaskCore.kt](file://app/src/main/java/io/legado/app/model/AutoTaskCore.kt)
- [app/src/main/java/io/legado/app/help/CronScheduleHelper.kt](file://app/src/main/java/io/legado/app/help/CronScheduleHelper.kt)
- [app/src/main/java/io/legado/app/utils/CronUtils.kt](file://app/src/main/java/io/legado/app/utils/CronUtils.kt)
- [app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt](file://app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt)
- [app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt](file://app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt)
- [rust/legado-core/src/auto_task.rs](file://rust/legado-core/src/auto_task.rs)
- [rust/legado-core/src/cron.rs](file://rust/legado-core/src/cron.rs)
- [rust/legado-db/src/repository/auto_task_repository.rs](file://rust/legado-db/src/repository/auto_task_repository.rs)
- [rust/legado-db/src/migration/migrations.rs](file://rust/legado-db/src/migration/migrations.rs)
- [app/src/test/java/io/legado/app/model/AutoTaskCoreTest.kt](file://app/src/test/java/io/legado/app/model/AutoTaskCoreTest.kt)
- [app/src/test/java/io/legado/app/model/AutoTaskSchedulePolicyTest.kt](file://app/src/test/java/io/legado/app/model/AutoTaskSchedulePolicyTest.kt)
- [app/src/test/java/io/legado/app/utils/CronScheduleTest.kt](file://app/src/test/java/io/legado/app/utils/CronScheduleTest.kt)
</cite>

## 更新摘要
**所做更改**   
- 更新了下载管理器相关章节，反映大量代码变更（222行新增，260行删除）
- 增强了下载处理、错误恢复和任务管理功能的文档说明
- 补充了下载任务与调度系统的集成机制
- 更新了故障排查指南中的下载相关问题

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向"任务调度系统"的完整文档，覆盖定时任务框架（Cron 表达式解析、触发与执行）、自动任务管理（注册、依赖、重试）、优先级队列（排序、资源分配、并发控制）、监控（日志、统计、告警）以及持久化（状态保存、恢复、迁移）。同时提供配置示例与扩展开发指南，帮助读者快速理解并基于现有代码进行二次开发。

**更新** 本次更新重点反映了下载管理器的重大改进，包括增强的下载处理机制、改进的错误恢复策略和优化后的任务管理功能。

## 项目结构
该任务调度系统在 Android 应用层与 Rust 核心层协同实现：
- Android 层负责服务生命周期、UI 交互、调度策略与 Cron 工具封装。
- Rust 层提供高性能 Cron 解析、任务模型与数据库仓库访问。
- 数据层通过 Room DAO 与 SQLite 完成持久化，Rust 侧通过 Repository 抽象统一读写。

```mermaid
graph TB
subgraph "Android 应用层"
App["App.kt<br/>应用初始化"]
Service["AutoTaskService.kt<br/>后台服务"]
Core["AutoTaskCore.kt<br/>调度核心"]
CronH["CronScheduleHelper.kt<br/>调度辅助"]
CronU["CronUtils.kt<br/>Cron 工具"]
Entity["AutoTaskEntity.kt<br/>任务实体"]
Dao["AutoTaskDao.kt<br/>Room DAO"]
end
subgraph "Rust 核心层"
AutoTaskRS["auto_task.rs<br/>任务模型/接口"]
CronRS["cron.rs<br/>Cron 解析/计算"]
Repo["auto_task_repository.rs<br/>任务仓库"]
Migrate["migrations.rs<br/>数据库迁移"]
DownloadMgr["download_manager.rs<br/>下载管理器"]
end
App --> Service
Service --> Core
Core --> CronH
Core --> CronU
Core --> Entity
Core --> Dao
Core --> Repo
Repo --> Migrate
Repo --> CronRS
Repo --> AutoTaskRS
Repo --> DownloadMgr
```

**图表来源** 
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/service/AutoTaskService.kt](file://app/src/main/java/io/legado/app/service/AutoTaskService.kt)
- [app/src/main/java/io/legado/app/model/AutoTaskCore.kt](file://app/src/main/java/io/legado/app/model/AutoTaskCore.kt)
- [app/src/main/java/io/legado/app/help/CronScheduleHelper.kt](file://app/src/main/java/io/legado/app/help/CronScheduleHelper.kt)
- [app/src/main/java/io/legado/app/utils/CronUtils.kt](file://app/src/main/java/io/legado/app/utils/CronUtils.kt)
- [app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt](file://app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt)
- [app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt](file://app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt)
- [rust/legado-core/src/auto_task.rs](file://rust/legado-core/src/auto_task.rs)
- [rust/legado-core/src/cron.rs](file://rust/legado-core/src/cron.rs)
- [rust/legado-db/src/repository/auto_task_repository.rs](file://rust/legado-db/src/repository/auto_task_repository.rs)
- [rust/legado-db/src/migration/migrations.rs](file://rust/legado-db/src/migration/migrations.rs)

## 核心组件
- 调度服务（AutoTaskService）：管理任务服务的生命周期、启动/停止、前台通知与线程池隔离。
- 调度核心（AutoTaskCore）：聚合任务注册、依赖解析、优先级队列、触发器与执行器，协调 Cron 计算与重试策略。
- Cron 解析与计算（CronUtils / CronScheduleHelper / cron.rs）：解析标准 Cron 表达式，计算下次触发时间，支持时区与边界条件处理。
- 任务实体与 DAO（AutoTaskEntity / AutoTaskDao）：定义任务字段、状态机与持久化操作。
- Rust 任务模型与仓库（auto_task.rs / auto_task_repository.rs）：跨语言任务模型、CRUD 与事务封装。
- 下载管理器（download_manager.rs）：**新增** 处理下载任务的创建、状态跟踪、错误恢复和并发控制。
- 数据库迁移（migrations.rs）：版本演进与兼容性保障。

**更新** 下载管理器的增强功能现在与任务调度系统深度集成，提供更可靠的下载任务处理能力。

## 架构总览
下图展示从应用启动到任务触发的端到端流程，包括服务初始化、任务加载、Cron 计算、队列入队与执行。

```mermaid
sequenceDiagram
participant App as "应用(App.kt)"
participant Svc as "服务(AutoTaskService.kt)"
participant Core as "核心(AutoTaskCore.kt)"
participant Cron as "Cron解析(cron.rs/CronUtils.kt)"
participant Repo as "仓库(auto_task_repository.rs)"
participant Download as "下载管理器(download_manager.rs)"
participant DB as "数据库(Room/Rust DB)"
participant Exec as "执行器(线程池)"
App->>Svc : 启动服务
Svc->>Core : 初始化调度器
Core->>Repo : 加载待执行任务
Repo->>DB : 查询任务列表
DB-->>Repo : 返回任务集合
Core->>Cron : 计算下次触发时间
Cron-->>Core : 返回触发时间
Core->>Download : 处理下载任务
Download->>Exec : 提交下载任务
Note over Core,Exec : 依赖图解析后拓扑排序
Exec-->>Core : 执行结果回调
Core->>Repo : 更新状态/统计/日志
```

**图表来源** 
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/service/AutoTaskService.kt](file://app/src/main/java/io/legado/app/service/AutoTaskService.kt)
- [app/src/main/java/io/legado/app/model/AutoTaskCore.kt](file://app/src/main/java/io/legado/app/model/AutoTaskCore.kt)
- [rust/legado-core/src/cron.rs](file://rust/legado-core/src/cron.rs)
- [app/src/main/java/io/legado/app/utils/CronUtils.kt](file://app/src/main/java/io/legado/app/utils/CronUtils.kt)
- [rust/legado-db/src/repository/auto_task_repository.rs](file://rust/legado-db/src/repository/auto_task_repository.rs)

## 详细组件分析

### Cron 表达式解析与触发机制
- 解析能力：支持标准 Cron 字段（秒/分/时/日/月/周），包含通配符、步长、范围与列表。
- 触发计算：根据当前时间与表达式计算下一次触发时刻，考虑闰年、月末与跨月边界。
- 时区与夏令时：在 Android 层使用系统时区，Rust 层提供稳定计算逻辑，避免歧义。
- 错误处理：非法表达式返回明确错误码，便于上层记录与回退策略。

```mermaid
flowchart TD
Start(["开始"]) --> Parse["解析 Cron 表达式"]
Parse --> Valid{"表达式合法?"}
Valid --> |否| Error["返回解析错误"]
Valid --> |是| NextTick["计算下次触发时间"]
NextTick --> TZ["应用时区与夏令时规则"]
TZ --> Validate["校验边界条件"]
Validate --> Done(["结束"])
Error --> Done
```

**图表来源** 
- [rust/legado-core/src/cron.rs](file://rust/legado-core/src/cron.rs)
- [app/src/main/java/io/legado/app/utils/CronUtils.kt](file://app/src/main/java/io/legado/app/utils/CronUtils.kt)
- [app/src/main/java/io/legado/app/help/CronScheduleHelper.kt](file://app/src/main/java/io/legado/app/help/CronScheduleHelper.kt)

### 自动任务管理与依赖关系
- 任务注册：支持动态注册与静态配置加载，重复注册会合并或拒绝。
- 依赖关系：有向无环图（DAG）建模，构建阶段进行拓扑排序，确保前置任务完成后才触发后续任务。
- 失败重试：指数退避与最大重试次数限制，支持可配置的重试策略与死信队列。
- 状态机：任务状态包括待执行、运行中、成功、失败、已取消等，状态变更需原子写入。

```mermaid
classDiagram
class AutoTask {
+id : String
+name : String
+cronExpr : String
+priority : Int
+status : Enum
+retryCount : Int
+maxRetries : Int
+dependencies : String[]
+nextTriggerTime : Long
+execute() Result
}
class TaskRegistry {
+register(task : AutoTask) void
+unregister(id : String) void
+getById(id : String) AutoTask?
}
class DependencyGraph {
+addEdge(from : String, to : String) void
+topologicalSort() String[]
+validateAcyclic() bool
}
class RetryPolicy {
+shouldRetry(task : AutoTask) bool
+backoffDelay(task : AutoTask) Long
}
AutoTask --> DependencyGraph : "依赖"
TaskRegistry --> AutoTask : "管理"
RetryPolicy --> AutoTask : "评估"
```

**图表来源** 
- [app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt](file://app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt)
- [app/src/main/java/io/legado/app/model/AutoTaskCore.kt](file://app/src/main/java/io/legado/app/model/AutoTaskCore.kt)
- [rust/legado-core/src/auto_task.rs](file://rust/legado-core/src/auto_task.rs)

### 优先级队列与并发控制
- 任务排序：基于优先级数值与到期时间双键排序，高优先级且即将到期的任务优先出队。
- 资源分配：为不同任务类型分配独立线程池，避免相互阻塞；CPU 密集型与 IO 密集型分离。
- 并发控制：令牌桶或信号量限流，防止瞬时峰值导致资源耗尽；支持全局与任务级并发上限。
- 背压与丢弃：当队列饱和时，依据策略丢弃低优先级任务或等待。

```mermaid
flowchart TD
Enqueue["任务入队"] --> PriorityCheck["比较优先级与到期时间"]
PriorityCheck --> QueueFull{"队列已满?"}
QueueFull --> |是| Backpressure["背压策略(丢弃/等待)"]
QueueFull --> |否| Insert["插入优先级队列"]
Insert --> Dequeue["取出最高优先级任务"]
Dequeue --> Execute["提交到对应线程池执行"]
Execute --> Update["更新状态与统计"]
```

### 下载任务管理
**新增** 下载管理器作为任务调度的重要组成部分，提供以下功能：

- **下载任务创建**：支持批量创建下载任务，自动分配任务ID和优先级
- **状态跟踪**：实时监控下载进度、速度和剩余时间
- **错误恢复**：断点续传、网络异常自动重试、文件完整性校验
- **并发控制**：限制同时进行的下载数量，避免资源竞争
- **存储管理**：智能选择存储路径，自动清理临时文件

```mermaid
flowchart TD
Create["创建下载任务"] --> Validate["验证URL和参数"]
Validate --> Queue["加入下载队列"]
Queue --> ConcurrencyCheck{"检查并发限制"}
ConcurrencyCheck --> |超过限制| Wait["等待可用槽位"]
ConcurrencyCheck --> |允许| Start["开始下载"]
Start --> Progress["监控下载进度"]
Progress --> Success{"下载成功?"}
Success --> |是| Verify["文件完整性校验"]
Success --> |否| ErrorHandle["错误处理与重试"]
Verify --> Complete["完成任务"]
ErrorHandle --> Retry{"达到最大重试次数?"}
Retry --> |否| Resume["恢复下载"]
Retry --> |是| Fail["标记失败"]
Resume --> Progress
Fail --> Complete
```

**图表来源** 
- [rust/legado-core/src/download_manager.rs](file://rust/legado-core/src/download_manager.rs)

### 任务监控：日志、统计与告警
- 执行日志：记录任务开始/结束、耗时、异常堆栈与上下文参数，支持分级输出与采样。
- 性能统计：QPS、平均/分位耗时、成功率、失败率、重试次数与队列长度指标。
- 告警规则：阈值触发（如连续失败、超时、队列积压），通过通知通道上报。
- 可视化：提供 API 或 UI 面板查看实时指标与历史趋势。

### 任务持久化：状态保存、恢复与迁移
- 状态保存：每次状态变更（创建、运行、成功、失败、取消）均落库，保证幂等与一致性。
- 恢复机制：服务重启后扫描未完成的任务，重新计算触发时间并恢复执行。
- 数据迁移：通过 migrations.rs 管理版本升级，兼容旧数据结构，提供增量脚本。
- 备份与导出：支持批量导出任务配置与执行历史，便于审计与迁移。

```mermaid
sequenceDiagram
participant Core as "调度核心"
participant Repo as "任务仓库"
participant Download as "下载管理器"
participant DB as "数据库"
participant Mig as "迁移模块"
Core->>Repo : 保存任务状态
Repo->>DB : 写入事务
DB-->>Repo : 确认
Note over Core,Repo : 服务重启
Core->>Repo : 加载未完成任务
Repo->>DB : 查询状态=运行/失败
DB-->>Repo : 返回任务集
Repo->>Download : 恢复下载任务
Download->>DB : 检查下载状态
DB-->>Download : 返回下载信息
Repo->>Mig : 检查版本与迁移
Mig-->>Repo : 迁移完成
Repo-->>Core : 返回恢复后的任务
```

**图表来源** 
- [rust/legado-db/src/repository/auto_task_repository.rs](file://rust/legado-db/src/repository/auto_task_repository.rs)
- [rust/legado-db/src/migration/migrations.rs](file://rust/legado-db/src/migration/migrations.rs)
- [app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt](file://app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt)

## 依赖关系分析
- Android 层依赖：AutoTaskCore 依赖 Cron 工具与 DAO；AutoTaskService 管理生命周期与线程池。
- Rust 层依赖：auto_task_repository 依赖 cron 解析与 auto_task 模型；migrations 负责版本演进。
- 耦合度：Core 与 Repo 解耦，通过接口调用；Cron 解析集中在 Rust 层，Android 层仅做适配。

```mermaid
graph LR
AppKt["App.kt"] --> ServiceKt["AutoTaskService.kt"]
ServiceKt --> CoreKt["AutoTaskCore.kt"]
CoreKt --> CronUtils["CronUtils.kt"]
CoreKt --> CronHelper["CronScheduleHelper.kt"]
CoreKt --> Entity["AutoTaskEntity.kt"]
CoreKt --> Dao["AutoTaskDao.kt"]
CoreKt --> RepoRS["auto_task_repository.rs"]
RepoRS --> CronRS["cron.rs"]
RepoRS --> AutoTaskRS["auto_task.rs"]
RepoRS --> Migrate["migrations.rs"]
RepoRS --> DownloadMgr["download_manager.rs"]
```

**图表来源** 
- [app/src/main/java/io/legado/app/App.kt](file://app/src/main/java/io/legado/app/App.kt)
- [app/src/main/java/io/legado/app/service/AutoTaskService.kt](file://app/src/main/java/io/legado/app/service/AutoTaskService.kt)
- [app/src/main/java/io/legado/app/model/AutoTaskCore.kt](file://app/src/main/java/io/legado/app/model/AutoTaskCore.kt)
- [app/src/main/java/io/legado/app/utils/CronUtils.kt](file://app/src/main/java/io/legado/app/utils/CronUtils.kt)
- [app/src/main/java/io/legado/app/help/CronScheduleHelper.kt](file://app/src/main/java/io/legado/app/help/CronScheduleHelper.kt)
- [app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt](file://app/src/main/java/io/legado/app/data/entity/AutoTaskEntity.kt)
- [app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt](file://app/src/main/java/io/legado/app/data/dao/AutoTaskDao.kt)
- [rust/legado-db/src/repository/auto_task_repository.rs](file://rust/legado-db/src/repository/auto_task_repository.rs)
- [rust/legado-core/src/cron.rs](file://rust/legado-core/src/cron.rs)
- [rust/legado-core/src/auto_task.rs](file://rust/legado-core/src/auto_task.rs)
- [rust/legado-db/src/migration/migrations.rs](file://rust/legado-db/src/migration/migrations.rs)

## 性能考量
- Cron 计算复杂度：单次计算 O(1) 近似，批量计算 O(n)，建议缓存最近触发时间。
- 队列操作：优先级队列插入与弹出 O(log n)，注意避免频繁重排。
- 线程池隔离：IO 密集与 CPU 密集分离，降低锁竞争与上下文切换开销。
- 背压策略：合理设置队列容量与丢弃策略，避免内存溢出。
- 批处理与合并：对高频小任务进行合并执行，减少调度开销。
- **下载优化**：**新增** 下载任务采用分块传输、连接复用和智能缓存，显著提升大文件下载效率。

## 故障排查指南
- Cron 表达式无效：检查语法与边界值，参考测试用例验证。
- 任务不触发：确认依赖是否满足、优先级是否正确、队列是否被背压丢弃。
- 执行失败：查看日志级别与堆栈，定位外部依赖问题；检查重试策略是否达到上限。
- 状态不一致：核对事务写入顺序与幂等性，必要时手动修复状态。
- 迁移失败：检查版本号与脚本兼容性，回滚或补全迁移步骤。
- **下载失败**：**新增** 检查网络连接、存储空间和权限设置；查看下载日志中的错误码；确认目标路径可写。
- **下载中断**：**新增** 启用断点续传功能；检查网络稳定性；验证文件完整性。

**更新** 新增了下载相关的故障排查指南，涵盖常见的下载问题和解决方案。

## 结论
本任务调度系统以 Android 服务与 Rust 核心协作的方式，实现了高可靠、可扩展的定时任务框架。通过标准化的 Cron 解析、严格的依赖管理与优先级队列，结合完善的监控与持久化机制，能够满足复杂业务场景下的自动化需求。**更新** 最新的下载管理器增强功能进一步提升了系统的稳定性和性能，特别是在处理大量下载任务时的表现更加出色。建议在扩展时遵循现有接口约定，保持 Cron 解析与任务模型的稳定性，并通过测试覆盖关键路径。

## 附录
- 配置示例
  - Cron 表达式：分钟/小时/日/月/周的组合，例如每 5 分钟执行一次。
  - 任务优先级：数值越小优先级越高，默认值为中等。
  - 重试策略：最大重试次数与退避间隔，建议不超过 3 次。
  - 线程池：为 IO 密集型任务分配较大线程数，CPU 密集型限制并发。
  - **下载配置**：**新增** 最大并发下载数、超时设置、重试策略和存储路径配置。
- 扩展开发指南
  - 新增任务类型：继承任务基类，实现 execute 方法，注册到调度核心。
  - 自定义 Cron 解析：在 Rust 层扩展解析器，Android 层适配调用。
  - 监控接入：实现日志与指标上报接口，接入统一监控平台。
  - 持久化扩展：在 DAO 与 Repository 中增加字段与查询方法，同步更新迁移脚本。
  - **下载扩展**：**新增** 实现自定义下载处理器，支持新的协议和存储后端。

**更新** 新增了下载配置和扩展开发的相关指导，帮助开发者更好地利用下载管理器的新功能。