...
participant API as "API模块(backup/cache/bookshelf)"
...
### 本地缓存策略
- SharedPreferences：适合存储小量配置、开关、用户偏好等键值对。
...
## 结论
本项目通过Flutter + Rust FFI + SQLite的组合，实现了高性能、可扩展且跨平台一致的数据持久化方案。仓库层清晰划分领域职责，迁移与备份机制完善，缓存策略灵活。遵循本文的最佳实践与排障指南，可有效保障数据一致性、性能与安全。
