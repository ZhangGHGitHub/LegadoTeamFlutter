...
## 更新摘要
**变更内容**   
- 更新了Android构建配置以支持新的MediaSessionBridge功能
- 更新了Flutter依赖管理以兼容新功能
- 增强了媒体会话桥接功能的构建流程说明
- 完善了依赖版本兼容性配置
...
**更新** 本次更新重点介绍了新增的MediaSessionBridge功能对构建系统的影响，以及相应的依赖管理和兼容性配置。
...
K --> D
K --> I
L["MediaSessionBridge<br/>新增功能"] --> E
L --> D
```
...
## 架构总览
下图展示从源码到产物的端到端构建链路，包括Android、Flutter、Rust与Web的交互关系，以及新增的MediaSessionBridge功能。
...
participant MSB as "MediaSessionBridge"
...
- 构建目标
  ...
  - 资源合并、代码混淆、签名校验
  - **新增** MediaSessionBridge功能集成
- 关键脚本与配置
  - 顶层与app模块Gradle脚本定义依赖、插件、变体与签名
  - 下载脚本用于获取外部依赖与资源
  - **更新** android/app/build.gradle.kts支持新的MediaSessionBridge
- 典型流程
  - 清理缓存 -> 解析依赖 -> 编译Kotlin/Java -> 集成MediaSessionBridge -> 资源打包 -> 签名 -> 产物输出
...
Compile --> MSBCheck{"是否包含MediaSessionBridge?"}
MSBCheck --> |是| IntegrateMSB["集成MediaSessionBridge"]
MSBCheck --> |否| Resources["合并资源与清单"]
IntegrateMSB --> Resources
...
- 构建目标
  ...
  - **新增** MediaSessionBridge功能支持
- 关键脚本与配置
  - Makefile统一入口，封装各平台构建命令
  - Android侧使用KTS Gradle脚本，其他平台使用CMake/Xcode
  - **更新** pubspec.yaml声明Flutter依赖以支持新功能
- 典型流程
  - 拉取依赖 -> 生成桥接代码 -> 编译Rust -> 集成MediaSessionBridge -> 构建目标平台 -> 产物归档
...
Cargo --> RustLib : "编译"
FRB --> RustLib : "导出API"
MediaSessionBridge --> FlutterApp : "媒体会话支持"
FlutterApp --> FRB : "调用绑定"
```
...
participant MSB as "MediaSessionBridge验证"
...
BUILD->>MSB : 验证MediaSessionBridge功能
MSB-->>BUILD : 功能验证结果
BUILD-->>ART : 产出包与日志
...
- Gradle依赖
  ...
  - **更新** android/app/build.gradle.kts包含MediaSessionBridge相关依赖
- Flutter依赖
  - **更新** pubspec.yaml声明Flutter插件与第三方库以支持新功能
...
AndGradle --> MSB["MediaSessionBridge依赖"]
...
- Android构建问题
  ...
  - 确认签名密钥库路径与密码正确
  - 清理缓存后重试构建
  - **新增** MediaSessionBridge相关依赖冲突排查
- Flutter构建问题
  - 确保Flutter SDK与插件版本兼容
  - 检查Rust工具链与目标平台配置
  - 查看平台特定错误日志（Xcode/CMake/NDK）
  - **更新** pubspec.yaml依赖版本兼容性检查
- Rust构建问题
...
## 结论
Legado项目通过Gradle、Flutter、Rust与Vite的组合，实现了跨平台应用的统一构建与持续交付。借助GitHub Actions与Dependabot，团队能够自动化测试、构建与发布，保证质量与效率。**本次更新重点增强了MediaSessionBridge功能的构建支持，并通过pubspec.yaml的依赖管理确保了新功能的兼容性**。遵循本文档的环境搭建、构建流程、依赖管理与发布规范，可有效提升团队协作与交付稳定性。
...
- Android
  ...
  - 安装Android SDK与命令行工具
  - 配置ANDROID_HOME与PATH
  - **新增** MediaSessionBridge相关Android依赖配置
- Flutter
  - 安装Flutter SDK并加入PATH
  - 安装平台工具（Xcode、CMake、NDK等）
  - 启用相应平台支持
  - **更新** pubspec.yaml依赖版本管理
- Rust
  - 安装rustup与目标工具链
...
- Android APK构建
  ...
  - **新增** MediaSessionBridge集成配置：[flutter_legado/android/app/build.gradle.kts](file://flutter_legado/android/app/build.gradle.kts)
- Flutter多平台构建
  ...
  - **更新** 依赖管理配置：[flutter_legado/pubspec.yaml](file://flutter_legado/pubspec.yaml)
- Rust交叉编译
