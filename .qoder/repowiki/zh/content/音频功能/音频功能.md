...
I["AudioIdleResumeTest.kt"]
J["AudioOfflineCachePlaybackTest.kt"]
K["ReadAloudFollowPositionRegressionTest.kt"]
L["SpeechNavigationGateTest.kt"]
end
E --> A
F --> D
B --> C
...
## 架构总览
整体采用分层架构：上层通过FFI接口调用Rust核心能力；核心层将TTS、缓存、预加载与朗读流程解耦；Android侧通过测试与UI驱动具体行为。
...
TEST_P["AudioPlaybackPreferenceTest.kt"] --> FFI_API
...
## 结论
Legado的音频功能以Rust核心为基础，通过清晰的模块化设计与完善的测试覆盖，实现了稳定的TTS朗读、灵活的播放控制、高效的缓存与预加载、以及可靠的后台服务。建议在部署前根据设备特性调优缓存与预加载策略，并结合用户反馈持续优化朗读体验。
