...
- [app/src/main/java/io/legado/app/service/McpService.kt](file://app/src/main/java/io/legado/app/service/McpService.kt)
...
## 更新摘要
**所做更改**
- 新增独立MCP服务端口管理系统章节，详细说明setMcpPort API实现
- 更新服务启动与生命周期章节，包含独立MCP服务启动流程
- 新增MCP服务架构图和端口管理流程图
- 更新依赖关系分析，反映新的MCP服务组件
- 补充故障排查指南中的MCP端口相关常见问题
...
**更新** 新增了独立MCP服务端口管理系统，支持通过BookApi的setMcpPort方法动态配置MCP服务端口，实现与主Web服务器的完全隔离。
...
subgraph "MCP 服务"
MCPSrv["独立MCP服务<br/>端口管理"]
MCPHandlers["mcp.rs<br/>MCP处理器"]
end
subgraph "网络层"
...
- [rust/legado-server/src/state.rs](file://rust/legado-server/src/state.rs)
- [rust/legado-server/src/error.rs](file://rust/legado-server/src/error.rs)
- [rust/legado-server/src/handlers/mcp.rs](file://rust/legado-server/src/handlers/mcp.rs)
- [rust/legado-net/src/middleware.rs](file://rust/legado-net/src/middleware.rs)
- [rust/legado-net/src/client.rs](file://rust/legado-net/src/client.rs)
- [rust/legado-net/src/rate_limit.rs](file://rust/legado-net/src/rate_limit.rs)
- [rust/legado-net/src/request.rs](file://rust/legado-net/src/request.rs)
...
- **新增** 独立MCP服务：提供独立的MCP服务端口管理，支持与主Web服务器完全隔离运行。
...
- **新增** 独立MCP服务启动：通过`serve_mcp`函数启动独立的MCP服务，使用专用的TCP监听器和路由配置，与主Web服务完全分离。
...
### 独立MCP服务端口管理
**新增** 独立MCP服务端口管理系统，提供以下核心功能：

- **端口配置验证**：支持端口范围1024-65530，默认值1236，越界时自动回退到默认值
- **独立服务启动**：通过`serve_mcp`函数启动专用MCP服务，使用独立的TCP监听器
- **路由隔离**：仅暴露`/mcp/tools`（GET）和`/mcp/call`（POST）端点，与健康检查`/health`
- **安全边界**：监听器绑定到本机回环地址127.0.0.1，确保服务安全性
- **状态共享**：与主Web服务共享同一AppState，避免重复初始化

```mermaid
flowchart LR
Client["客户端请求"] --> PortCheck["端口验证"]
PortCheck --> Valid{"端口有效?"}
Valid --> |是| StartMCP["启动MCP服务"]
Valid --> |否| Error["返回错误"]
StartMCP --> BindListener["绑定TCP监听器"]
BindListener --> CreateRouter["创建MCP路由器"]
CreateRouter --> Serve["提供服务"]
Serve --> Health["健康检查 /health"]
Serve --> Tools["工具列表 /mcp/tools"]
Serve --> Call["工具调用 /mcp/call"]
```

**图表来源** 
- [rust/legado-server/src/server.rs:81-99](file://rust/legado-server/src/server.rs#L81-L99)
- [app/src/main/java/io/legado/app/service/McpService.kt:168-170](file://app/src/main/java/io/legado/app/service/McpService.kt#L168-L170)

**章节来源**
- [rust/legado-server/src/server.rs:53-99](file://rust/legado-server/src/server.rs#L53-L99)
- [app/src/main/java/io/legado/app/service/McpService.kt:168-170](file://app/src/main/java/io/legado/app/service/McpService.kt#L168-L170)
- [docs/API_CONTRACT.md:321-323](file://docs/API_CONTRACT.md#L321-L323)
...
- **新增** MCP服务依赖：独立MCP服务通过专用监听器与路由配置，与主Web服务保持松耦合。
...
Server --> MCPServer["独立MCP服务"]
MCPServer --> MCPHandlers["mcp.rs"]
...
- 并发与调度
  ...
  - 批量操作合并请求，减少往返次数。
- **新增** MCP服务性能优化
  - 独立端口隔离：MCP服务与主Web服务完全分离，避免相互影响
  - 端口复用：支持在同一进程中运行多个服务实例
  - 资源隔离：MCP服务使用独立的监听器和路由配置
- 监控与度量
...
- 常见问题
  ...
  - **新增** MCP端口冲突：检查1236端口是否被占用，确认端口范围1024-65530有效性
...
- 诊断步骤
  ...
  - **新增** MCP服务状态检查：通过`/health`端点验证MCP服务健康状态
  - 使用压测工具模拟峰值流量，观察错误率与延迟变化。
- 恢复策略
  - 自动重试与熔断降级，保护下游服务。
  - 优雅关闭与滚动重启，避免服务中断。
  - **新增** MCP服务重启：通过setMcpPort API动态重启MCP服务

**章节来源**
- [rust/legado-server/src/error.rs](file://rust/legado-server/src/error.rs)
- [rust/legado-net/src/client.rs](file://rust/legado-net/src/client.rs)
- [rust/legado-js/src/sandbox.rs](file://rust/legado-js/src/sandbox.rs)
- [rust/legado-server/src/server.rs:93-94](file://rust/legado-server/src/server.rs#L93-L94)

## 结论
本服务端架构以 Actix-web 为核心，结合模块化设计实现了高内聚、低耦合的请求处理体系。通过统一的状态容器、中间件链、错误与日志规范，以及完善的网络层与脚本引擎支持，系统在可维护性、可扩展性与性能方面具备良好基础。**新增的独立MCP服务端口管理系统**进一步增强了服务的灵活性和隔离性，允许MCP服务在主Web服务之外独立运行，提供更好的资源管理和故障隔离能力。建议在生产环境中持续完善监控告警、容量规划与自动化运维能力，进一步提升稳定性与可用性。

## 附录
- 部署配置要点
  - 环境变量：端口、日志级别、数据库连接串、外部服务地址、密钥等。
  - **新增** MCP配置：MCP端口配置（默认1236，范围1024-65530）、独立服务开关、安全边界配置
