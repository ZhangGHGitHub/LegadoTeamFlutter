...
## 更新摘要
**所做更改**   
- 新增QUIC/HTTP3协议支持章节，详细说明现代网络协议的优势与配置
- 更新架构总览图，包含QUIC连接路径
- 增强性能考量部分，添加QUIC协议的性能优化建议
- 更新故障排除指南，包含QUIC相关问题的排查方法
- 补充依赖关系分析，展示QUIC模块的集成方式
...
**最新更新**：网络层新增了QUIC/HTTP3协议支持，增强了现代Web标准兼容性，提供比传统TCP HTTP更好的连接复用和延迟性能。

## 项目结构
网络层位于Rust模块legado-net中，采用模块化设计：以client为核心入口，围绕request/response数据模型，通过middleware链式组装能力（重试、代理、SSL、UA、验证等），配合cookie_store进行会话持久化，rate_limit控制并发与速率，verification与user_agent提供通用增强，url_template支持动态URL生成，cover/rss/direct_link_upload/webdav等子模块封装特定业务场景的HTTP交互。新增的quic.rs模块提供QUIC/HTTP3协议支持。
...
## 架构总览
下图展示一次典型HTTP请求从应用层到网络层的完整流转，包括中间件链的执行顺序、重试与代理的应用、Cookie注入与响应解析，以及QUIC/HTTP3协议的可选路径。
...
MW->>UA : "注册UA注入"
MW->>Verify : "注册签名/校验"
Client->>QUIC : "尝试QUIC连接"
alt QUIC可用
QUIC-->>Client : "QUIC连接成功"
else QUIC不可用
Client->>TCP : "回退到TCP连接"
TCP-->>Client : "TCP连接建立"
end
...
class HTTPClient {
+setQUICConfig(config) void
+checkQUICSupport(url) bool
+negotiateProtocol(url) Protocol
}
HTTPClient --> QUICConfig : "配置"
```
...
## 依赖关系分析
网络层内部模块间耦合清晰：client作为聚合器依赖request/response模型，通过middleware编排各能力模块；cookie_store与rate_limit为横向支撑；verification与user_agent提供通用增强；url_template服务于请求构建；quic模块提供现代协议支持；业务子模块复用client能力完成特定场景。
...
**更新**：新增QUIC协议优化建议，利用现代网络协议提升整体性能。
...
## 结论
Legado的网络层以client为核心，通过middleware灵活组合重试、代理、SSL、UA、校验等能力，配合cookie_store与rate_limit实现会话保持与并发控制。新增的QUIC/HTTP3协议支持进一步提升了网络性能，特别是在移动网络和弱网环境下表现更佳。模块化设计使业务子模块可复用统一能力，提升开发效率与系统稳定性。建议在复杂场景下结合监控与日志持续优化性能与可靠性。

**更新**：强调了QUIC/HTTP3协议支持带来的性能提升和现代Web标准兼容性。
