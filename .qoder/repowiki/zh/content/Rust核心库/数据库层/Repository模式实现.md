...
- 仓库聚合(repository/mod.rs)
  ...
  - 作为上层模块的统一入口
- 领域仓库实现
  - book_repository.rs：书籍实体的CRUD、搜索、分页、批量操作
  - book_source_repository.rs：书源实体的增删改查与状态管理
...
- [legado-db/src/repository/book_repository.rs](file://rust/legado-db/src/repository/book_repository.rs)
- [legado-db/src/repository/book_source_repository.rs](file://rust/legado-db/src/repository/book_source_repository.rs)
- [legado-db/src/repository/rss_source.rs](file://rust/legado-db/src/repository/rss_source_repository.rs)
- [legado-db/src/repository/cache_repository.rs](file://rust/legado-db/src/repository/cache_repository.rs)
- [legado-db/src/repository/search_book_repository.rs](file://rust/legado-db/src/repository/search_book_repository.rs)
...
Tx-->>Repo : "成功"
Repo-->>Caller : "返回结果"
Note over Repo,Tx : "若任一阶段失败，回滚并抛出异常"
```
...
```mermaid
flowchart TD
Start(["写入缓存"]) --> CheckKey["检查键是否存在"]
CheckKey --> Exists{"存在?"}
Exists --> |是| Update["更新值与过期时间"]
...
sequenceDiagram
participant Caller as "调用方"
participant Repo as "SearchBookRepository"
participant Tx as "事务管理器"
participant DB as "数据库连接"
Caller->>Repo : "搜索(关键词,页码,大小)"
Repo->>DB : "执行全文检索SQL"
DB-->>Repo : "返回结果集"
Repo->>Repo : "应用分页与排序"
Repo-->>Caller : "返回分页结果"
...
## 结论
Legado项目通过Repository模式实现了数据访问层的清晰分层与统一抽象。各仓库提供一致的CRUD接口，结合查询构建器与批处理机制，既保证了代码的可维护性，又提升了系统性能。未来可进一步引入更复杂的查询DSL与异步I/O，以应对更大规模的数据访问需求。
