...
## 结论
解析引擎通过统一的规则分析器整合了 XPath、正则表达式与 JSONPath 的能力，配合 HTML 解析与 JSON 数据处理，形成完整的“规则驱动的数据抽取”体系。借助 AST 生成、执行计划优化与缓存策略，系统在准确性与性能之间取得平衡。遵循本文档的编写指南与最佳实践，可有效提升规则的稳定性与可维护性。
