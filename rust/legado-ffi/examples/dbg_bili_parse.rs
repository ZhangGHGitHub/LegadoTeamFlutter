...
async fn run(db_path: &str, query: &str, out: &mut String) {
    ...
    let sources = match legado_ffi::api::source::list_enabled_sources() {
        Ok(v) => v,
        Err(e) => { w!("sources err: {e}"); return; }
    };
    let Some(source) = sources.iter().find(|s| s.book_source_name.contains("哔哩哔哩")).cloned() else {
    ...
    let template = match &source.search_url {
        Some(t) => t.clone(),
        None => { w!("no searchUrl"); return; }
    };
    let analyzed =
        legado_ffi::js_executor::build_search_url_with_setup(
            &template,
            query,
            1,
    ...
    let client = match legado_net::client::LegadoClient::new(Default::default()) {
        Ok(c) => c,
        Err(e) => { w!("client err: {e}"); return; }
    };
    let resp = match client.get(analyzed.url(), None).await {
        Ok(r) => r,
        Err(e) => { w!("GET err: {e}"); return; }
    };
    ...
    if let Ok(els) = analyzer.get_elements(full) {
        if let Some(first_el) = els.first() {
            ...
            let rules: Vec<(&str, &str)> = vec![
                ...
                ("bookUrl-base", "https://api.bilibili.com/x/web-interface/view?aid={{$.id}}"),
            ];
            for (fname, rule) in &rules {
                match item.get_string(rule) {
                    Ok(s) => w!("[field {}] => {}", fname, s.chars().take(150).collect::<String>()),
