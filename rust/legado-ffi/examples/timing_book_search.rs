...
fn main() {
    ...
    for r in &reqs {
        let port = if r.url.starts_with("https://") {
            443
        } else {
            80
        };
    ...
    let do_probes = std::env::var("LEGADO_NET_TIMING_PROBES")
        .map(|v| v == "1")
        .unwrap_or(false);
    if do_probes {
        legado_ffi::runtime::block_on(async {
            for (host, port) in &hosts {
                let p = legado_net::timing::probe_tcp_connect(host, *port, Duration::from_secs(5))
                    .await;
    ...
    // stdout 仅 ASCII 摘要
    println!(
        "wall={:.2}s batches={} requests={} hosts={} -> {}",
        wall,
        batch_lines.len(),
        reqs.len(),
        hosts.len(),
